...
function FileSelected(file){
  ...
  var thumbPreview = RoxyUtils.GetUrlParam('preview');
  var dialog = RoxyUtils.GetUrlParam('dialog');
  var is_other = RoxyUtils.GetUrlParam('is_other');
  if(!is_other) {
    $(window.parent.document).find('#' + fieldId).attr('value', file.fullPath);
    $(window.parent.document).find('#' + thumbPreview).attr('src', file.fullPath);
  } else {
    window.parent.addOtherImage(file.fullPath);
  }
  window.parent.closeCustomRoxy(dialog);
